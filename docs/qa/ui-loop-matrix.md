...
| settings.vllm.model_switch_small@vllm | Models → Use with sidecar (small) | `assert_heavy_op_allowed('Qwen/Qwen3-1.7B')`; switch + restart | served name re-derived from new model; `/v1/models` lists it; chat works | vllm | pass | Phase 2: switch endpoint accepts whitelisted Qwen3-1.7B (200); /v1/models lists it + chat works (batch 4); real restart proven by live running state |
...
| settings.vllm.served_name_resync@vllm | Settings → served name on model change | switch model without custom name; GET config | `OMNI_SERVED_MODEL_NAME` re-derived from new model tail | vllm | pass | Phase 2: omni_model->Qwen2.5-7B-Instruct re-derives served name to 'Qwen2.5-7B-Instruct' (compose-gen, no restart; restored) |
| settings.vllm.hf_offline@vllm | Settings → HF offline toggle | switch to cached model; inspect compose env | `HF_HUB_OFFLINE`/`TRANSFORMERS_OFFLINE` set for cached model | vllm | pass | Phase 2: switching to a cached model sets hf_offline=true in compose env (restored) |
