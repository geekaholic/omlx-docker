...
Run `omni serve --help`, `omni status --help`, `omni logs --help`, or
`omni restart --help` for argparse's detailed option descriptions.
""".strip()
...
def write_env_file(path: Path, values: Mapping[str, str]) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    lines = [
        "# Generated by omni. Edit with `omni serve` flags or update this file locally.",
    ]
...
def vllm_env_from_compose(path: Path) -> dict[str, str]:
    if not path.exists():
        return {}
    values: dict[str, str] = {}
...
def merged_vllm_environment(
    ...
    *,
    ...
    if env_file is not None:
        values.update(_known_vllm_env(load_env_file(env_file)))
    if env_file is not None and not env_file.exists() and compose_file is not None:
        values.update(_known_vllm_env(vllm_env_from_compose(compose_file)))
...
def vllm_settings_from_env(values: Mapping[str, str]) -> VllmComposeSettings:
    ...
    return VllmComposeSettings(
        ...
        hf_home=values.get("VLLM_HF_HOME", _host_path(defaults.hf_home)),
...
def _known_vllm_env(values: Mapping[str, str]) -> dict[str, str]:
...
def vllm_settings_from_args(
    ...
    return vllm_settings_from_env(
        ...
        else {**default_vllm_environment(), **_known_vllm_env(existing_env), **vllm_cli_environment(args)}
    )
