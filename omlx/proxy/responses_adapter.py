...
def responses_to_chat_body(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for field in ("temperature", "top_p", "top_k", "presence_penalty", "frequency_penalty"):
...
async def stream_responses_events(
    ...
    yield _sse("response.created", {"type": "response.created", "response": _base_resp("in_progress", [])})
    ...
    try:
        ...
        yield _sse("error", {"type": "error", "error": {"message": str(exc), "code": "server_error"}})
        return
