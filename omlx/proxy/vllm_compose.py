...
def settings_from_overrides(overrides: dict[str, Any]) -> VllmComposeSettings:
    ...
    kwargs.update(
        ...
        tokenizer_mode=str(
            pick("vllm_tokenizer_mode", defaults.tokenizer_mode)
        ).strip(),
        ...
        kv_cache_dtype=str(
            pick("vllm_kv_cache_dtype", defaults.kv_cache_dtype)
        ).strip(),
...
def vllm_environment(settings: VllmComposeSettings) -> dict[str, str]:
    return {
        **common_environment(settings),
        ...
        "VLLM_ENABLE_CHUNKED_PREFILL": _optional_bool_str(
            settings.enable_chunked_prefill
        ),
        "VLLM_ENABLE_PREFIX_CACHING": _optional_bool_str(
...
def vllm_settings_from_env(values: Mapping[str, str]) -> VllmComposeSettings:
    ...
    kwargs.update(
        ...
        generation_config=values.get(
            "VLLM_GENERATION_CONFIG", defaults.generation_config
        ),
        ...
        uvicorn_log_level=values.get(
            "VLLM_UVICORN_LOG_LEVEL", defaults.uvicorn_log_level
        ),
...
def write_vllm_env_file(
    path: str | os.PathLike[str], values: Mapping[str, str]
) -> Path:
...
def render_vllm_compose(
    ...
    *,
    ...
    env_reload = render_env_reload_snippet(DEFAULT_VLLM_ENV_NAME)
...
{env_lines}
      ...
      HF_HUB_OFFLINE: "${{OMNI_HF_OFFLINE:-false}}"
      TRANSFORMERS_OFFLINE: "${{OMNI_HF_OFFLINE:-false}}"
    volumes:
...
{env_reload}
        ...
        set -- "$${{@}}" --enable-prompt-tokens-details
        ...
        unset OMLX_PROXY_PORT OMLX_PROXY_BIND_HOST OMLX_PROXY_API_KEY OMLX_BACKEND_API_KEY OMLX_CONTEXT_SCALING OMLX_TARGET_CONTEXT_SIZE OMLX_SSE_KEEPALIVE_MODE OMLX_SAMPLING_MAX_TOKENS OMLX_SAMPLING_TEMPERATURE OMLX_SAMPLING_TOP_P OMLX_SAMPLING_TOP_K OMLX_SAMPLING_REPETITION_PENALTY
        exec vllm serve "$${{@}}"
...
def write_vllm_compose(
    path: str | os.PathLike[str], settings: VllmComposeSettings
) -> Path:
    output = Path(path)
